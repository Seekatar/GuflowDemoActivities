...
COPY ./CCC.CAS.Workflow3Activities/CCC.CAS.Workflow3Activities.csproj CCC.CAS.Workflow3Activities/
...
WORKDIR /src/CCC.CAS.Workflow3Activities
RUN dotnet restore

COPY . .

RUN export PATH="$PATH:/root/.dotnet/tools" && \
     dotnet sonarscanner begin /k:"CAS-Workflow3Activities" /d:sonar.host.url="https://sonarqube.cccis.com" /d:sonar.login=$sonarQubeToken /v:$version; \
     exit 0

RUN dotnet publish "CCC.CAS.Workflow3Activities/CCC.CAS.Workflow3Activities.csproj" -c Release -o /app -p:Version=$version,AssemblyVersion=$version
...
ENTRYPOINT ["dotnet", "CCC.CAS.Workflow3Activities.dll"]
